...
test("Add a new ship", () => {
  expect(gameboard.placeShip("ship", ["0-0", "0-1"])).toEqual([true, []]);
});

test("Add a new ship, outbound test", () => {
  expect(gameboard.placeShip("ship", ["0-10"])).toEqual([false, []]);
});

test("Add new ships", () => {
  expect(gameboard.placeShip("ship", ["0-0", "0-1"])).toEqual([true, []]);
  expect(gameboard.placeShip("ship2", ["2-0", "2-1"])).toEqual([true, []]);
});
...
test("Add new ships, duplicated ship test", () => {
  expect(gameboard.placeShip("ship", ["0-0", "0-1"])).toEqual([true, []]);
  expect(gameboard.placeShip("ship", ["2-0", "2-1"])).toEqual([true, ["0-0", "0-1"]]);
});

test("Add new ships, add ships test, occupied test", () => {
  expect(gameboard.placeShip("ship", ["0-0", "0-1"])).toEqual([true, []]);
  expect(gameboard.placeShip("ship2", ["0-0", "2-1"])).toEqual([false, []]);
});

test("Add new ships, duplicated ship test, occupied test", () => {
  expect(gameboard.placeShip("ship", ["0-0", "0-1"])).toEqual([true, []]);
  expect(gameboard.placeShip("ship", ["2-0", "2-1"])).toEqual([true, ["0-0", "0-1"]]);
  expect(gameboard.placeShip("ship2", ["0-0"])).toEqual([true, []]);
  expect(gameboard.placeShip("ship3", ["2-1"])).toEqual([false, []]);
});
...
test("Receive attack test, success attack test", () => {
  ...
  expect(gameboard.placeShip(ship1, ["0-0", "0-1"])).toEqual([true, []]);
...
test("Lose test, losed test", () => {
  ...
  expect(gameboard.placeShip(ship1, ["0-0", "0-1"])).toEqual([true, []]);
  expect(gameboard.placeShip(ship2, ["1-0", "1-1"])).toEqual([true, []]);
  expect(gameboard.receiveAttack("0-0")).toEqual(ship1);
  ship1.hit();
